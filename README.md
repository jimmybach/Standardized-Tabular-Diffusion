...
Adapter presence is not a release claim. Run `python -m standardized_tabular_diffusion.cli list-models --details` to inspect source authority, modification status, validation level, benchmark track, and support level separately. At this stage, adapters remain experimental and unsupported. CTAB-GAN+, CTGAN, TVAE, NRGBoost, TabDDPM, TabDiff, TabSyn, and SMOTE have passed separate Linux/Python 3.11 native-parity protocols, but that evidence does not make them benchmark-eligible or release-supported.

CTAB-GAN now targets method-author commit `73d4e315` under Apache-2.0. The former 15-file semantic fork has been replaced by seven checksum-locked official files; the adapter preserves the official stratified split, exposes classification only, and uses a documented compatibility bridge that forwards the unchanged mixture-component count to scikit-learn's keyword-only API. The complete local six-case parity protocol passes, while authoritative Linux evidence is pending; see the [CTAB-GAN validation protocol](docs/CTABGAN_VALIDATION.md) and [Chinese translation](docs/CTABGAN_VALIDATION.zh-CN.md).
