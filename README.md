...
P5 adds five-seed Random-Forest C2ST with raw AUROC and a separate label-invariant fidelity complement, exact training-collision and internal-duplication audits, source-attested SDMetrics DCR distributions with heldout calibration, and a declared black-box DOMIAS KDE membership-inference attack. High-order fidelity and empirical privacy remain separate, no overall score or formal privacy guarantee is emitted, and unresolved metrics are machine-readable exclusions. The complete identity-surrogate path passed on the reviewed Adult and Sick tables under Windows 11/Python 3.11, with [implementation evidence](docs/evidence/evaluation/p5-windows-py311-identity-c66fa23.json) retained. The first real-generator pilot then trained the checksum-pinned TabDDPM Adult `ddpm_cb_best` configuration for 30,000 steps on an RTX 5080, generated three 32,561-row tables at seeds `0,1,2`, and finalized all three complete P5 bundles; its bounded [machine-readable evidence](docs/evidence/evaluation/p5-tabddpm-adult-windows-py311-a2e4f27.json) is retained. Adult/Sick roles and the bounded black-box threat model are now reviewed, while attribute inference remains excluded. `p5-high-order-privacy@1.0.0` is preregistered for an independent seeds `3,4,5` confirmation before a separate freeze decision; see the [P5 guide](docs/evaluation/P5_HIGH_ORDER_PRIVACY.md).
