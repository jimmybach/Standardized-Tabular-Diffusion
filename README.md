...
P5 adds five-seed Random-Forest C2ST with raw AUROC and a separate label-invariant fidelity complement, exact training-collision and internal-duplication audits, source-attested SDMetrics DCR distributions with heldout calibration, and a declared black-box DOMIAS KDE membership-inference attack. High-order fidelity and empirical privacy remain separate, no overall score or formal privacy guarantee is emitted, and unresolved metrics are machine-readable exclusions. The complete identity-surrogate path passed on the reviewed Adult and Sick tables under Windows 11/Python 3.11, with [machine-readable evidence](docs/evidence/evaluation/p5-windows-py311-identity-c66fa23.json) retained. `p5-high-order-privacy@0.1.0` remains diagnostic pending generator pilots, dataset-specific privacy-role review, and later protocol freeze; see the [P5 guide](docs/evaluation/P5_HIGH_ORDER_PRIVACY.md).
