...
Goggle targets checksum-locked, unpatched method-author source from `vanderschaarlab/GOGGLE` commit `1a3d87ad`, acquired on demand under MIT. The GCN core retains exact native-parity evidence from Linux/DGL run [`30945676747`](https://github.com/jimmybach/Standardized-Tabular-Diffusion/actions/runs/30945676747). The ordinary Windows runtime now replaces the unavailable DGL dependency with the explicitly recorded `standardized-goggle-torch-graph@1.0.0` compatibility backend; DGL remains only as an independent validation oracle. This is a dependency compatibility reimplementation, not an unmodified official DGL path. SAGE and heterogeneous decoding fail closed; see the [Goggle validation protocol](docs/GOGGLE_VALIDATION.md) and [Chinese translation](docs/GOGGLE_VALIDATION.zh-CN.md).
...
CTGAN now targets the checksum-pinned official `ctgan==0.12.1` package instead of the legacy embedded source snapshot. Its mandatory Linux/Python 3.11 protocol passed all exact comparisons for three independent training/sampling seed pairs in [run `32047234665`](https://github.com/jimmybach/Standardized-Tabular-Diffusion/actions/runs/32047234665), so its status is `native-parity-validated`. The official package uses BUSL-1.1; see the [CTGAN validation protocol](docs/CTGAN_VALIDATION.md) before installation or use.

TVAE now targets `ctgan.TVAE` from that same checksum-pinned official package. The locally modified `0.5.2.dev0` snapshot and its obsolete wrappers have been removed. Its mandatory Linux/Python 3.11 protocol passed all exact comparisons for three independent training/sampling seed pairs in [run `32052308431`](https://github.com/jimmybach/Standardized-Tabular-Diffusion/actions/runs/32052308431), and the immutable evidence is retained, so its status is `native-parity-validated`; see the [TVAE validation protocol](docs/TVAE_VALIDATION.md).
...
Original project code and documentation are licensed under [Apache-2.0](LICENSE). This root license does not relicense included or downloaded third-party source, packages, datasets, papers, or model weights; consult [NOTICE](NOTICE), [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md), and each component's own terms. Contribution and attribution records live in [CONTRIBUTING.md](CONTRIBUTING.md), [CONTRIBUTORS.md](CONTRIBUTORS.md), [SECURITY.md](SECURITY.md), [docs/DATA_GOVERNANCE.md](docs/DATA_GOVERNANCE.md), and the [upstream source audit](docs/UPSTREAM_SOURCE_AUDIT.md). Third-party redistribution and dataset-specific release decisions remain independent release gates.
...
P4 adds checksum-bound real-train, held-out real-test, and synthetic-train inputs; three-family Local Utility with raw Dummy/TRTR/TSTR arms and unclipped baseline-adjusted retention; and TabStruct Equation 4 Global Utility with every target, raw arm, predictor set, seed, denominator, and failure state retained. Its row-order-invariant successor passed the complete native Windows 11/Python 3.11/RTX 5080 schedule: 67 tasks, 134 arms, all six five-seed identity sentinels exactly `1.0`, and every preregistered execution/resource gate. `p4-utility@1.0.0` is now protocol-frozen for conditional Official Results use on that exact environment. Dataset, model, track, run, and release admission remain independent, and no generator quality was assessed; see the [P4 guide](docs/evaluation/P4_UTILITY.md), [freeze decision](docs/evidence/evaluation/p4-protocol-freeze-decision-2026-08-12.json), and [retained successor evidence](docs/evidence/evaluation/p4-dataset-scale-windows-gpu-stable-6dc485f.json).

P5 adds five-seed Random-Forest C2ST with raw AUROC and a separate label-invariant fidelity complement, exact training-collision and internal-duplication audits, source-attested SDMetrics DCR distributions with heldout calibration, and a declared black-box DOMIAS KDE membership-inference attack. High-order fidelity and empirical privacy remain separate, no overall score or formal privacy guarantee is emitted, and unresolved metrics are machine-readable exclusions. After the earlier Windows implementation and exploratory generator gates, a clean committed preregistration fixed a fresh TabDDPM/Adult run at generation seeds `3,4,5`. All three 32,561-row tables passed the structural, complete metric-scope, provenance, and bundle-finalization gates on Windows 11/Python 3.11/RTX 5080 without using numerical scores for the decision. `p5-high-order-privacy@1.0.0` is now protocol-frozen for conditional use only; Adult, Sick, TabDDPM, concrete runs, and release support remain separately unadmitted. See the [P5 guide](docs/evaluation/P5_HIGH_ORDER_PRIVACY.md) and [freeze decision](docs/evidence/evaluation/p5-protocol-freeze-decision-2026-08-13.json).
...
- `goggle` uses checksum-locked method-author source plus a narrow pure-PyTorch GCN graph backend on Windows. DGL 1.1.3 is a validation-only oracle; SAGE and heterogeneous decoding are explicitly unsupported.
