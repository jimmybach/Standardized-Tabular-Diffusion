...
P4 adds checksum-bound real-train, held-out real-test, and synthetic-train inputs; three-family Local Utility with raw Dummy/TRTR/TSTR arms and unclipped baseline-adjusted retention; and TabStruct Equation 4 Global Utility with every target, raw arm, predictor set, seed, denominator, and failure state retained. Historical validation isolated a row-order-sensitive hidden AutoGluon split as the remaining stability defect. P4 now exposes one result-producing successor: complete training rows are canonicalized before an explicit seeded fit/tuning split, while exact TabEval execution remains internal provenance validation only. The successor passed the complete native Windows 11/Python 3.11/RTX 5080 schedule: 67 tasks, 134 arms, all six five-seed identity sentinels exactly `1.0`, and every preregistered execution/resource gate. Historical failures remain immutable, and profile freeze plus Official Results admission still require separate review; see the [P4 guide](docs/evaluation/P4_UTILITY.md) and [retained successor evidence](docs/evidence/evaluation/p4-dataset-scale-windows-gpu-stable-6dc485f.json).
