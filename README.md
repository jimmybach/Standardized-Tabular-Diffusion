...
Native-Windows real-function coverage is tracked separately in the [cross-baseline pipeline audit](docs/audits/PIPELINE_REAL_FUNCTION_AUDIT.md) and its [Chinese translation](docs/audits/PIPELINE_REAL_FUNCTION_AUDIT.zh-CN.md). The plan combines inexpensive contract simulation across all 21 adapters with a mandatory minimal authoritative-algorithm run for every non-blocked adapter; full representative three-seed runs remain targeted rather than universal.
