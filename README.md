...
P5 adds five-seed Random-Forest C2ST with raw AUROC and a separate label-invariant fidelity complement, exact training-collision and internal-duplication audits, source-attested SDMetrics DCR distributions with heldout calibration, and a declared black-box DOMIAS KDE membership-inference attack. High-order fidelity and empirical privacy remain separate, no overall score or formal privacy guarantee is emitted, and unresolved metrics are machine-readable exclusions. `p5-high-order-privacy@0.1.0` remains diagnostic pending Windows dataset-scale evidence and dataset-specific privacy review; see the [P5 guide](docs/evaluation/P5_HIGH_ORDER_PRIVACY.md).
