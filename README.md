...
Adapter presence is not a release claim. Run `python -m standardized_tabular_diffusion.cli list-models --details` to inspect source authority, modification status, validation level, benchmark track, and support level separately. At this stage, adapters remain experimental and unsupported. CTAB-GAN, CTAB-GAN+, CTGAN, TVAE, NRGBoost, TabDDPM, TabDiff, TabSyn, STaSy, and SMOTE have passed separate Linux/Python 3.11 native-parity protocols, but that evidence does not make them benchmark-eligible or release-supported.
...
CoDi now targets the checksum-locked baseline snapshot distributed by the same TabSyn commit. All 11 CoDi files and the complete 24-file execution scope are fail-closed; checkpoints are isolated under `output_dir`, and exact requested sample rows are supported without changing tracked upstream source. The separate method-author repository has no declared license and differs in five of ten shared paths, so original-method claims are blocked. Linux/Python 3.11 snapshot-parity evidence is pending; see the [CoDi validation protocol](docs/CODI_VALIDATION.md) and [Chinese translation](docs/CODI_VALIDATION.zh-CN.md).
