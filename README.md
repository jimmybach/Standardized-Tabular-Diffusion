...
P4 adds checksum-bound real-train, held-out real-test, and synthetic-train inputs; three-family Local Utility with raw Dummy/TRTR/TSTR arms and unclipped baseline-adjusted retention; and TabStruct Equation 4 Global Utility with every target, raw arm, predictor set, seed, denominator, and failure state retained. Historical dataset-scale validation isolated a row-order-sensitive hidden AutoGluon split as the remaining stability defect. P4 now exposes one result-producing successor: complete training rows are canonicalized before an explicit seeded fit/tuning split, while exact TabEval execution remains internal provenance validation only. The historical failures remain immutable, and the successor still requires complete Windows 11/Python 3.11/RTX 5080 validation before profile freeze or Official Results admission; see the [P4 guide](docs/evaluation/P4_UTILITY.md).
