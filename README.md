...
P3 adds benchmark-native per-column and reviewed cross-column Validity, original-output evidence, a closed declarative rule language, and the explicit train-only mean/mode preprocessing boundary. Generated data is never silently repaired. Current observed numerical ranges and unresolved semantic or medical constraints are not promoted to hard rules. P3 has passed its local exit gates and awaits retained Linux/Python 3.11 evidence; see the [P3 guide](docs/evaluation/P3_VALIDITY_AND_PREPROCESSING.md).
