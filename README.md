...
P4 adds checksum-bound real-train, held-out real-test, and synthetic-train inputs; three-family Local Utility with raw Dummy/TRTR/TSTR arms and unclipped baseline-adjusted retention; and TabStruct Equation 4 Global Utility with every target, raw arm, predictor set, seed, denominator, and failure state retained. The implementation is a unit-validated diagnostic pilot, not an Official Results protocol or an executable TabEval source-parity claim. The pinned optional XGB/KNN/TabPFN runtime still requires the Linux/Python 3.11 source-runtime pilot and profile freeze described in the [P4 guide](docs/evaluation/P4_UTILITY.md).
