...
from standardized_tabular_diffusion.models.next_wave_baselines import CTABGANPlusAdapter
from standardized_tabular_diffusion.upstream_sources import source_manifest_path, validate_upstream_source

PROTOCOL_ID = "ctabgan-plus-native-parity-v1"
...
UPSTREAM_TREE = "f5a08d81b0309d6635bf1c7a646965a34913fa93"
UPSTREAM_MODEL_TREE = "645e6a9d5129346f5d4e29085f1bafd5de4531fd"
SEED_CASES = (0, 19, 73)
...
def _fixture_frame(variant: str) -> pd.DataFrame:
    ...
    if variant == "classification":
        frame["target"] = np.asarray(["no", "yes"])[(row % 5 == 0).astype(int)]
    elif variant == "regression":
        frame["target"] = np.round(0.35 * frame["count"] - 0.8 * frame["continuous"] + row / 50.0, 4)
    else:
...
def _write_fixture(root: Path, variant: str) -> tuple[pd.DataFrame, DatasetSpec, dict[str, Any]]:
    ...
    atomic_write_bytes(
        metadata_path,
        (json.dumps({"name": f"ctabgan-plus-{variant}-parity", "task_type": variant}, indent=2) + "\n").encode(),
    )
    ...
    spec = DatasetSpec(
        name=f"ctabgan-plus-{variant}-parity",
        task_type=variant,
...
def _official_parameters(dataset_spec: DatasetSpec) -> dict[str, Any]:
    ...
    task_name = "Classification" if dataset_spec.task_type == "classification" else "Regression"
    return {
        "test_ratio": 0.2,
        "categorical_columns": categorical,
        "log_columns": [],
...
def _model_signature(model: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "configuration": {
            ...
            "general_columns": model.general_columns,
            "non_categorical_columns": model.non_categorical_columns,
...
def _run_native(
    ...
    checkpoint_path = output_dir / "ctabgan_plus.pkl"
...
def _run_adapter(
    ...
    checkpoint_metadata = json.loads(
        (output_dir / "ctabgan_plus.pkl.metadata.json").read_text(encoding="utf-8")
    )
    ...
    sample_metadata = json.loads((output_dir / "ctabgan_plus_sample_metadata.json").read_text(encoding="utf-8"))
...
def run_validation(repo_root: Path, source_dir: Path, output_dir: Path, evidence_path: Path) -> dict[str, Any]:
    ...
    for frame, dataset_spec, _ in fixtures:
        for seed in SEED_CASES:
            case_root = output_dir / dataset_spec.task_type / f"seed-{seed}"
            ...
            cases.append(
                {
                    "variant": dataset_spec.task_type,
                    "seed": seed,
    ...
    evidence = {
        ...
        "claim_boundary": (
            "Native adapter parity only. The upstream repository declares no license, so source redistribution, "
            "benchmark eligibility, Official Results, and release support remain blocked."
        ),
        ...
        "environment_lock": {
            "path": "requirements-ctabgan-plus-validation.txt",
            "sha256": _sha256_file(repo_root / "requirements-ctabgan-plus-validation.txt"),
        },
