...
| `ctab-gan` | Adapter complete / checksum-locked official source | [Team-TUD/CTAB-GAN](https://github.com/Team-TUD/CTAB-GAN) | Seven selected files from pinned commit `73d4e315` replace the former semantic fork and retain Apache-2.0 attribution. The classification-only adapter preserves the official split and applies a documented non-semantic scikit-learn API bridge. All six local parity cases pass; authoritative Linux evidence is pending. |
