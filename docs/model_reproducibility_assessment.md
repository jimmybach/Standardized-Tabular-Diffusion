...
| `great` | Native parity validated against the official package | [tabularis-ai/be_great](https://github.com/tabularis-ai/be_great) at `b300f612`, PyPI `be-great==0.0.14` | The former vendored path is replaced by the checksum-locked official wheel. Training and guided sampling use unchanged APIs; the adapter adds typed input, scoped randomness, safe safetensors/JSON persistence, and integrity checks. All three exact tensor/sample/seed cases passed in retained Linux/Python 3.11 run `30974574472`. |
...
| `tabula` | Native parity validated against locked method-author source | [zhao-zilong/Tabula](https://github.com/zhao-zilong/Tabula) at `a7d34a94` | Six method-author files are checksum-locked and fetched on demand. The adapter invokes unchanged fit/sample code with safe persistence and a bounded, exact-row Linux sampling boundary. All three exact tensor/sample/seed cases passed in retained run `30974574505`. No upstream license is declared, so redistribution and release remain blocked. |
| `tabsds` | Native parity validated against locked method-author source | [echaibub/TabSDS](https://github.com/echaibub/TabSDS) at `86650149` | Two official notebook helpers replace the former approximation. Nine binary/multiclass/regression and seed cases passed exact source-versus-adapter parity in retained run `30974574593`, including repeat/truncate exact-row behavior. No upstream license is declared. |
...
| `goggle` | Method-author GCN core plus dependency compatibility backend | [vanderschaarlab/GOGGLE](https://github.com/vanderschaarlab/GOGGLE) at `1a3d87ad` | Eighteen official files remain checksum-locked and unpatched. Historical run `30945676747` retains exact DGL native parity for the GCN core. Windows uses the separately declared pure-PyTorch graph backend; run `32042446422` retains exact DGL 1.1.3 backend parity across five graph-level and nine end-to-end cases. SAGE/heterogeneous support, Official Results, and release gates remain pending. |
