...
Scope: the primary TabDDPM, TabDiff, and TabSyn source trees, the TabSyn STaSy and CoDi baseline snapshots, plus official integrations for ARF, BN/pgmpy, NFlow/nflows, CTGAN, TVAE, SMOTE, NRGBoost, REaLTabFormer, TabularARGN, CTAB-GAN, CTAB-GAN+, and Goggle
...
| BN (official pgmpy package + declared recipe) | `617cb48af678a7a471aad81d523ca95d2095430f`, tree `6c7adc00a479f540b2215889b1fac99a7b0b8a9c` (`v1.1.2`, `pgmpy==1.1.2`) | Trusted PyPI provenance binds the release to the tag commit. All 636 package files in the wheel match the source distribution and tag; the wheel, metadata, license, critical Git blobs, and installed files are checksum-locked. | No package source is vendored or patched. DatasetSpec typing, deterministic quantile discretization, explicit isolated nodes, scoped randomness, and typed JSON graph/CPD state are adapter-only. All nine exact cases passed in retained Linux/Python 3.11 run `30967779298`. | Native parity validated for the exact pgmpy package plus declared BIC/BDeu recipe, not a paper-native implementation; central evaluation, dataset admission, runtime, and release gates remain blocked. |
| NFlow (official nflows package + declared recipe) | `64b856c081e5f07521b32be99da262e8338fbfe8`, tree `83057958f8773e35044e3aa5c13ac9c06c4a3994` (`v0.14`, `nflows==0.14`) | The PyPI source distribution is checksum-locked and all 42 package files match the lightweight tag. The tag/commit and PyPI upload are unsigned, and the source distribution omits the tag's MIT license file while declaring MIT in metadata. | No package source is vendored or patched. Declared mixed-type preprocessing, a frozen MAF architecture/optimizer, scoped PyTorch state, strict data contracts, and non-executable JSON/NumPy persistence are adapter-only. All nine exact cases passed in retained Linux/Python 3.11 run `30970260840`. | Native parity validated for the exact canonical package plus declared recipe; central evaluation, dataset admission, runtime, and release gates remain blocked. This is not a paper-native implementation claim. |
...
- The canonical nflows 0.14 repository declares MIT. Its PyPI source distribution, lightweight tag commit/tree, all 42 package files, source-tag license hash, critical runtime files, and frozen validation recipe are locked. The PyPI archive omits `LICENSE.md`, uses neither Trusted Publishing nor a detached signature, and is not vendored. All nine exact official-package recipe-parity cases passed in retained run `30970260840`.
