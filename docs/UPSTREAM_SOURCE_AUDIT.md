...
Until all applicable gates are met, registry records remain conservative. CTAB-GAN, CTAB-GAN+, CTGAN, TVAE, NRGBoost, Goggle, TabDDPM, TabDiff, TabSyn, STaSy, and SMOTE have retained native-parity claims, but remain experimental, unsupported, and excluded from Official Results. Goggle's claim is limited to its method-author GCN core; STaSy's claim is limited to the TabSyn benchmark snapshot; SMOTE is additionally and categorically excluded from the joint generative-model ranking.
