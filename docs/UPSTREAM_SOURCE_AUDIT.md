...
Scope: the primary TabDDPM, TabDiff, and TabSyn source trees, the TabSyn STaSy and CoDi baseline snapshots, plus official integrations for ARF, BN/pgmpy, CTGAN, TVAE, SMOTE, NRGBoost, REaLTabFormer, TabularARGN, CTAB-GAN, CTAB-GAN+, and Goggle
...
| BN (official pgmpy package + declared recipe) | `617cb48af678a7a471aad81d523ca95d2095430f`, tree `6c7adc00a479f540b2215889b1fac99a7b0b8a9c` (`v1.1.2`, `pgmpy==1.1.2`) | Trusted PyPI provenance binds the release to the tag commit. All 636 package files in the wheel match the source distribution and tag; the wheel, metadata, license, critical Git blobs, and installed files are checksum-locked. | No package source is vendored or patched. DatasetSpec typing, deterministic quantile discretization, explicit isolated nodes, scoped randomness, and typed JSON graph/CPD state are adapter-only. The independent nine-case recipe-parity protocol passes locally; authoritative Linux/Python 3.11 evidence is pending. | Adapter-complete pending retained native-parity evidence. The target is the exact pgmpy package plus declared BIC/BDeu recipe, not a paper-native implementation; all Official Results and release gates remain blocked. |
