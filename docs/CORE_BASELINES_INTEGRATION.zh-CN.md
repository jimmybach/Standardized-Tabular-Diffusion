...
- 恢复 TabSyn 主实现源码本身不等于其单独附带的 CoDi baseline 已完成审计。CoDi 随后单独完成了 24 文件 checksum 审计并建立仅适配层的运行边界；九个 Linux/Python 3.11 TabSyn 快照精确一致性案例均在已保留的运行 `32043925805` 中通过。
