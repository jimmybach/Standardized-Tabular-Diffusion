...
- Restoring the primary TabSyn source did not by itself audit its separately vendored CoDi baseline. CoDi has now received a separate 24-file checksum audit and adapter-only runtime boundary; its dedicated Linux/Python 3.11 parity evidence remains pending.
