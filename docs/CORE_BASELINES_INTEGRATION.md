...
- Restoring the primary TabSyn source did not by itself audit its separately vendored CoDi baseline. CoDi subsequently received a separate 24-file checksum audit and adapter-only runtime boundary; all nine exact Linux/Python 3.11 TabSyn-snapshot parity cases passed in retained run `32043925805`.
