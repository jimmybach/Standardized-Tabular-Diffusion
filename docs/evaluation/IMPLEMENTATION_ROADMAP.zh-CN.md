...
完整 Adult/Sick identity-surrogate 路径已在提交 `c66fa23` 的 Windows 11/Python 3.11 环境通过，并留存[机器可读证据](../evidence/evaluation/p5-windows-py311-identity-c66fa23.json)。首个探索性真实生成器试验随后以锁定的 TabDDPM Adult `ddpm_cb_best` 配置训练 30,000 步，在种子 `0,1,2` 生成三张完整表，并完成三份 P5 bundle finalization。Adult/Sick 隐私角色与“发布表黑盒 + 辅助真实参照”的成员推断威胁模型随后完成审阅。独立生成种子 `3,4,5` 设计先写入[确认性预注册](../evidence/evaluation/p5-confirmatory-preregistration-2026-08-13.json)，再通过全部完整性、结构、来源和环境门槛。[冻结决定](../evidence/evaluation/p5-protocol-freeze-decision-2026-08-13.json)没有使用指标数值是否有利，只对十个指标组成部分作出有条件准入。
...
当前实现：fail-closed finalized-bundle 读取、精确科学兼容组、Atomic Result 到 suite 的聚合、确定性分层区间、显式缺失/失败与两两分母、独立准入、经审阅纠正、发布类别、不可变 manifest/fingerprint、同源 JSON/CSV/HTML/Markdown 文件、CLI 命令以及 Windows 主/Linux 次 CI 均已实现。该工程完成不会把任何现有结果自动提升为 Official。
...
当前实现：冻结旧 schema 与校验和绑定的“只导入不转换”边界、中央适配器评测路径、精确状态/证据 inventory、包内人工 SMOTE→P3→诊断快照快速开始、发布资产、中英双语 P8 指南及 Windows/Linux 发布工作流，均已为 `0.1.0rc1` 实现。旧记录和诊断快照都不能获得 Official rank。精确原生 Windows 11/Python 3.11 退出门已在实现提交 `bb09085` 通过，并留存[机器可读证据](../evidence/evaluation/p8-native-windows11-py311-bb09085.json)。托管 Windows 平台家族与 Linux 发布门已在[运行 31760027871](https://github.com/jimmybach/Standardized-Tabular-Diffusion/actions/runs/31760027871)通过，两份记录均由 [P8 契约](P8_MIGRATION_AND_RELEASE.zh-CN.md)链接留存。
