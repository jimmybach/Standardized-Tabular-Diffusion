...
- P5 已实现五种子平衡 C2ST、精确训练碰撞、内部重复、锁定 SDMetrics DCR 与留出校准，以及声明完整的黑盒 DOMIAS KDE 攻击；Windows 11/Python 3.11 Adult/Sick identity-surrogate 证据和首个 TabDDPM/Adult 三种子真实生成器试验证据均已留存。Adult/Sick 隐私角色和有边界的成员推断威胁模型已经审阅；P5 v1 已预注册独立生成种子 `3,4,5` 确认性试验，之后再另行作出冻结决定。
...
| P5 | 高阶 Fidelity 与经验 Privacy 工作包 | P2、P3 | 冻结候选：数据集角色/威胁模型已审阅，Windows 证据与探索性试验已留存，独立确认性试验已预注册 | 严格运行生成种子 `3,4,5` 确认性设计，并记录不依赖分数好坏的冻结决定 |
...
完整 Adult/Sick identity-surrogate 路径已在提交 `c66fa23` 的 Windows 11/Python 3.11 环境通过，并留存[机器可读证据](../evidence/evaluation/p5-windows-py311-identity-c66fa23.json)。首个探索性真实生成器试验随后以锁定的 TabDDPM Adult `ddpm_cb_best` 配置训练 30,000 步，在种子 `0,1,2` 生成三张完整表，并完成三份 P5 bundle finalization；其[机器可读证据](../evidence/evaluation/p5-tabddpm-adult-windows-py311-a2e4f27.json)也已留存。Adult/Sick 隐私角色与“发布表黑盒 + 辅助真实参照”的成员推断威胁模型现已审阅。独立生成种子 `3,4,5` 设计已写入[确认性预注册](../evidence/evaluation/p5-confirmatory-preregistration-2026-08-13.json)；后续冻结决定只能依据完整性、结构、来源和环境门槛，不能依据指标数值是否有利。
...
P4 已完成条件式协议冻结，P5 v1 已完成角色/威胁模型审阅并预注册独立确认性试验。该试验及另行作出的冻结决定完成后，下一项实现增量是 P6：先定义执行节点身份契约、硬件/资源 profile、时间与内存测量、安全 cache key 和可安全处理中断的 resume 语义，再编写 orchestration 代码。
