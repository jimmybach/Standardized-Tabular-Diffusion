...
| [`evaluation/utility.py`](../../standardized_tabular_diffusion/evaluation/utility.py) | held-out-test Local/Global Utility、原始 arms、支持状态与严格 ratio 聚合 | P4 已通过单元验证的诊断路径；TabEval 可执行运行时等价仍待验证 |
...
- P4 Local/Global Utility 已作为诊断性垂直切片通过单元验证；精确可选 TabEval 预测器运行时、资源预算、稳定性 pilot 与来源等价证据仍需在协议冻结前完成。
...
| P4 | Local 与 Global Utility | P1、P3 | 已通过单元验证的诊断实现；来源运行时 pilot 待完成 | 原始 arms、状态语义、profile 身份和来源/公式验证通过 |
