...
Decision: predictor profiles have distinct metric identities. The favorable constant-target fallback is not accepted by this benchmark; it becomes an explicit support failure. The TabStruct formula remains the Global Utility target. Exact TabEval execution is retained only as internal provenance validation. Historical dataset-scale runs showed that TabEval's `tuning_data=None` call allowed input row order to change AutoGluon's hidden validation split. The sole result-producing successor therefore canonicalizes complete training rows and supplies an explicit seeded AutoGluon fit/tuning split while preserving official XGB/KNN/TabPFN packages, metrics, and target coverage. This documented invocation change has a new identity and does not claim exact source parity. The profile remains diagnostic until the preregistered Windows GPU successor passes.
