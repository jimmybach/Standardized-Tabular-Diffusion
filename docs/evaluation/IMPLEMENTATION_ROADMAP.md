...
- P5 implements balanced five-seed C2ST, exact train collisions, internal duplication, exact pinned SDMetrics DCR with heldout calibration, and a declared black-box DOMIAS KDE attack. Its Windows 11/Python 3.11 Adult/Sick identity-surrogate evidence and first real-generator TabDDPM/Adult three-seed pilot evidence are retained. Adult/Sick privacy roles and the bounded membership threat model are reviewed; P5 v1 is preregistered for an independent seeds `3,4,5` confirmation before a separate freeze decision.
...
| P5 | High-order fidelity and empirical privacy work packages | P2, P3 | Freeze candidate with reviewed dataset roles/threat model, retained Windows evidence, exploratory pilot, and preregistered independent confirmation | Run the exact seeds `3,4,5` confirmatory design and record a non-score-based freeze decision |
...
P4 is conditionally protocol-frozen, and P5 v1 has completed role/threat-model review and is preregistered for an independent confirmatory run. After that run and its separate freeze decision, the next implementation increment is P6: define the execution-node identity contract, hardware/resource profiles, timing and memory measurements, safe cache keys, and interruption-safe resume semantics before adding orchestration code.
