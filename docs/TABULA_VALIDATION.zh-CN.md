...
官方构造、标签编码、行文本训练和采样调用保持不变。官方采样器只在内部行数循环结束后检查类别取值范围，因此最终可用行数可能少于请求数。适配器会针对剩余行数重复调用未修改的官方采样器，保持调用顺序与随机数流，并只在最终边界截断；连续返回零行会明确失败。适配器还约束 Python、NumPy 与 PyTorch 随机状态，隔离官方导入，并把训练文件放在 `output_dir` 下。由于官方采样循环没有重试上限，POSIX 使用外部计时器，Windows 则把同一个官方调用放入一次性子进程，由父进程施加硬超时。诊断环境仍可显式选择不安全的无界覆盖。

方法作者代码会在类别标签编码之前统计默认条件分布，但实际训练使用编码后的 ID。当调用者使用拟合得到的默认值时，适配器会在未修改的官方采样调用前，把保存的原始标签确定性映射为拟合标签编码器中的精确 ID。若专家显式提供 `start_col`/`start_col_dist`，则原样转发，从而保持冻结的等价性协议。

官方 `torch.save` 检查点在持久化边界被替换为 safetensors、类型化 JSON 标签编码器/状态，以及完整文件清单。恢复过程会保留方法作者 `load_from_dir()` 路径的训练模式语义，随后仍调用官方类的原始 `sample()`。危险扩展名、符号链接、清单外文件或字节变化均失败关闭。

## 强制等价用例

工作流离线构建单层 GPT-2，并在同一张 24 行全类别表上，以 0、19、73 三个种子分别训练官方直接路径和适配器路径。微型模型禁用提前 EOS 终止，避免让源码中写死的完整 GPT-2 填充 ID 回流到刻意缩小的嵌入表；两条路径使用完全相同的夹具。每个用例逐张量精确比较，重新加载安全产物，并在直接官方路径独立应用相同的精确行数边界；两条路径使用同一个编码目标分布调用未修改的官方采样器，最终要求 5 行 DataFrame 完全一致和 CSV 字节完全一致。
