...
原生路径直接调用 `main.py` 完成训练和采样，采样使用官方提供的 `--report --num_runs 1` 模式。原因是固定版本的普通 test 路径会把 DCR 的 NumPy 明细数组交给仅支持 DataFrame 和字典的保存分支；report 模式才是上游已经提供且能正确处理这些数组的路径。适配器调用完全相同的 report 模式，并把第一份生成样本映射到标准化产物清单。以下条件必须全部满足：
