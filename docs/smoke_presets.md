...
- `goggle-adult-smoke.json` uses checksum-locked method-author source and the recorded pure-PyTorch GCN graph backend. It tests integration rather than generation quality, needs neither DGL nor PyTorch Geometric, and does not support SAGE or heterogeneous decoding. The preset defaults to CPU; the Windows V2 plan overrides it to CUDA.
