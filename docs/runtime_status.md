...
- `tabdiff`: passed `tabdiff-native-parity-v1` in GitHub Actions run `30866879879`. The approved configurable-seed/config-path runtime boundaries then passed exact seed checks and a real Windows/Python 3.11/RTX 5080 Adult run: one 10.6M-parameter training, three complete 32,561-row seed outputs, strict schema validation, and finalized central P2/P3 bundles. The official source remains checksum-exact; see `docs/TABDIFF_VALIDATION.md` and the retained evidence. This is not an Official Results or release-support claim.
