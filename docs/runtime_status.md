...
- `goggle`: the method-author GCN core passed all nine binary, multiclass, regression, and seed cases in retained DGL protocol run `30945676747`. The current Windows runtime keeps that source unchanged and substitutes `standardized-goggle-torch-graph@1.0.0` for the unavailable DGL binary. Its RTX 5080 Windows V2 real-function probe passed training, two distinct seed outputs, strict data validation, checkpoint immutability, and central P3 bundle finalization at commit `0a23a84`. The dependency replacement still needs retained Linux/DGL 1.1.3 formal evidence; SAGE and heterogeneous decoding are rejected.
